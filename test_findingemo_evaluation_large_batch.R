#!/usr/bin/env Rscript
# FindingEmo Dataset Comparative Evaluation Script (batch, face=none, oai-large)

pkgload::load_all()

cat("🚀 FindingEmo Dataset Evaluation Script (batch, face=none, oai-large)\n")
cat("========================================================================\n\n")
...
cache_path <- file.path(TARGET_DIR, "predictions_cache-large-batch.rds")
use_cache <- FALSE
...
preds_csv <- file.path(TARGET_DIR, "predictions_all_variants-large-batch.csv")
write.csv(combined_predictions, preds_csv, row.names = FALSE)

metrics_csv <- file.path(TARGET_DIR, "metrics_comparison-large-batch.csv")
write.csv(metrics_comparison, metrics_csv, row.names = FALSE)

summary_file <- file.path(TARGET_DIR, "evaluation_comparative_summary-large-batch.txt")
sink(summary_file)
cat("FindingEmo Comparative Evaluation Summary (batch, face=none, oai-large)\n")
cat("============================================================================\n")
...
cat("Model: transforEmotion (oai-large, batch)\n")
...
cat("  - ", metrics_csv, "\n")
cat("  - ", summary_file, "\n\n")
cat("🎯 Batch large script completed successfully!\n")
