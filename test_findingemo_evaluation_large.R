#!/usr/bin/env Rscript
#' FindingEmo Dataset Comparative Evaluation Script (oai-large)
#' 
...
#' to assess their impact on downstream metrics using the CLIP oai-large model.
#' 
#' Steps:
...
#' 5. Save combined CSV and TXT summaries (with -large suffix)
...
cat("🚀 FindingEmo Dataset Evaluation Script (oai-large)\n")
cat("===============================================\n\n")
...
# Define constants
TARGET_DIR <- "./findingemo_eval_50"   # same dataset directory
MAX_IMAGES <- 50
...
# Use a separate cache for large model
cache_path <- file.path(TARGET_DIR, "predictions_cache-large.rds")
use_cache <- FALSE
...
preds_csv <- file.path(TARGET_DIR, "predictions_all_variants-large.csv")
write.csv(combined_predictions, preds_csv, row.names = FALSE)

# 6b. Metrics comparison CSV
metrics_csv <- file.path(TARGET_DIR, "metrics_comparison-large.csv")
write.csv(metrics_comparison, metrics_csv, row.names = FALSE)

# 6c. Comparative TXT summary
summary_file <- file.path(TARGET_DIR, "evaluation_comparative_summary-large.txt")
sink(summary_file)
cat("FindingEmo Comparative Evaluation Summary (oai-large)\n")
cat("====================================================\n")
...
cat("Model: transforEmotion (oai-large)\n")
