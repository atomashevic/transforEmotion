...
#'
#' @param text Character vector or list. Text to analyze. One entry per document.
...
#' @param labels_set Character vector of allowed labels. If \code{NULL}, defaults
...
#' @param response_mode Character. LlamaIndex response mode. Default \code{"compact"}.
#' @param output Character. \code{"table"} (default) or \code{"json"}.
#' @param global_analysis Logical. If TRUE, analyze all documents jointly. Default FALSE.
#' @param ... Additional arguments passed to \code{rag()} (e.g., \code{device}, \code{keep_in_env}).
#'
#' @return
#' For Gemma3-1B/4B and \code{output = "table"}/\code{"csv"}, a data.frame with columns\cr
#' \code{doc_id, text, label, confidence}.\cr
#' For Gemma3-1B/4B and \code{output = "json"}, a JSON array of per-doc objects with those fields.\cr
#' For other models, structured outputs are not supported; the function falls back to \code{output = "text"} and returns a free-text \code{"rag"} object.
#'
#' @examples
#' \dontrun{
#' texts <- c(
...
rag_sentemo <- function(
  ...
  query <- if (identical(task, "emotion")) {
    ...
    "Extract the overall sentiment (positive, neutral, negative) in each document."
  }
