...
## 1. Python setup using `setup_modules()`  {#section_1}

*transforEmotion* runs its transformer models in Python through the *reticulate* package. You do not need to install Python yourself: on first use, *reticulate* builds an isolated Python environment with [uv](https://docs.astral.sh/uv/), downloading uv itself if it is not installed. The environment does not interfere with any existing Python installation.

The first build downloads about 500 MB of Python packages. Later R sessions reuse the cached environment and start in under a second.

You can run the setup ahead of time with `setup_modules()`. It builds the environment and downloads the default text, image and sentence-similarity models (about 1 GB), so the first analysis does not wait on either:

```{r setup_modules, eval = FALSE, echo = TRUE}
setup_modules()
```

Running `setup_modules()` is optional; every function sets up what it needs on first use. Packages for optional features, such as `rag()`, are added when those functions are first called, or ahead of time with `setup_modules(extras = "rag")`.
...
As mentioned in section [1](#section_1), the Python environment is set up automatically the first time you run `transformer_scores`. The first run will also trigger the download of the [Cross-Encoder's DistilRoBERTa](https://huggingface.co/cross-encoder/nli-distilroberta-base) transformer model. Therefore, the easiest way to get started is by using an example.
